...
private:
	...
	long *pointers;							//指向文件中的数据，通过排序pointers，到达给文件中数据排序的目的！
	fstream outfile, overflow, sorted;		//outfile为散列表，overfile为溢出表，sorted为排序所用的临时文件！
...
unsigned long File::hash(char *s)					//取3组字符串，每组4个。采用除余法生成地址：address + counter*recordLen
{
...
void File::insertion(char line[])						//在散列表中插入数据，如果冲突，则放在overflow表中！（线性搜索）
{
	int address = hash(line), counter = 0;
	char name[recordLen + 1];
	bool done = false, inserted = false;
	outfile.clear();								//清除流的错误状态！
	outfile.seekg(address, ios::beg);				//对输入文件重定位（向文件写入流），第一个参数表示偏移量，第二个参数表示位置！
...
void File::excise(char line[])							//删除数据，将首字符标记为#！
{
	getName(line);
	int address = hash(line), counter = 0;
...
void File::partition(int low, int high, int& pivotLoc)			//快速排序
{	
...
void File::sortFile()						//将排序结果导入sorted文件！
{
...
void File::combineFiles()											//将outfile和overflow合并，等待排序输出！
{
...
void File::processFile(char *fileName)						//运行函数！
{
