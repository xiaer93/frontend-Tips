...
/*横幅左右滑动封装函数！*/
/*
    selector:       css选择器，传入左右移动父元素
    slideNumbers：  传入左右移动元素的数量
    margin：        传入元素间距，距离为百分数
    slideEasing：   传入动画的easing属性
    slideCallback： 传入动画执行后的回调函数（思路，在动画执行后的complete函数中，调用sideCallback函数，并传入相关信息！）
*/
